...
CREATE DATABASE Ispit_2024_07_12
GO
USE Ispit_2024_07_12
...
INSERT INTO Narudzbe(NarudzbaID,UposlenikID,DatumNarudzbe,KompanijeIme,AdresaKupca)
...
CREATE INDEX indx_pretraga ON Proizvodi(NazivProizvoda)
...
EXEC sp_update_proizvodi @ProizvodID=1,@NazivProizvoda='Kafa'
...
GO
CREATE FUNCTION f_4c(@NarudzbaID INT)
RETURNS TABLE
AS
RETURN 
SELECT
...
WHERE sn.NarudzbaID=@NarudzbaID

SELECT * FROM f_4c(10248)
SELECT * FROM StavkeNarudzbe

--d) (6 bodova) Pronaci najmanju narudzbu placenu karticom i isporuceno na porducje Europe, 
--uz id narudzbe prikazati i spojeno ime i prezime kupca te grad u koji je isporucena narudzba (AdventureWorks)
...
WHERE soh.CreditCardID IS NOT NULL AND st.[Group] = 'Europe'
ORDER BY soh.TotalDue

--e) (6 bodova) Prikazati ukupan broj porizvoda prema specijalnim ponudama.Potrebno je prebrojati samo one proizvode koji pripadaju 
--kategoriji odjece ili imaju zabiljezen model (AdventureWorks)
...
FROM (
	SELECT 
		soh.CustomerID,
		COUNT(soh.SalesOrderID) BrojNarudzbi
...
SELECT TOP 1
	p.ProductID,
	p.Name,
	sod.OrderQty KolicinaProdaje,
	YEAR(soh.OrderDate) Godina,
	DATEPART(QUARTER,soh.OrderDate) Kvartal
FROM AdventureWorks2017.Production.Product AS p
...
--b) (16 bodova) Prikazati 5 narudzbi sa najvise narucenih razlicitih proizvoda i 5 narudzbi sa najvise porizvoda koji pripadaju razlicitim
--potkategorijama. Upitom prikazati ime i prezime kupca, id narudzbe te ukupnu vrijednost narudzbe sa popoustom zaokruzenu na 2 decimale (AdventureWorks)
